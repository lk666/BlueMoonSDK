...
public class CommonEditText extends EditText {
    ...
    public void init() {
        nameModeTxt = mContext.getString(R.string.content_no_name);
        TextWatcher textWatcher = new TextWatcher() {
            @Override
            public void afterTextChanged(Editable s) {
                // TODO: lk 2016/11/24 逻辑有问题，应全部设置到 afterTextChanged
                if (s.length() > maxLength) {
                    if (mCallback != null) {
                        mCallback.outOfLengthShow(mContext, maxLength);
                    } else {
                        LibPublicUtil.showToast(mContext, String.format(mContext.getString(R
                                .string.error_max_limit), maxLength));
                    }
                    resetText = true;
                    setText(tmp);
                    ...
                    if (!resetText && s.toString().length() - tmp.length() > 0) {
                        ...
                        if (MODE_NAME == limitMode) {
                            if (str.contains("-") || str.contains("_")
                                    || str.contains(mContext.getResources().getString(R.string
                                    .limit_char_1)) || str.contains(mContext.getResources()
                                    .getString(R.string.limit_char_2))) {
                                str = str.replaceAll("-", "");
                                str = str.replaceAll("_", "");
                                str = str.replaceAll(mContext.getResources().getString(R.string
                                        .limit_char_1), "");
                                str = str.replaceAll(mContext.getResources().getString(R.string
                                        .limit_char_2), "");
                            }
                            ...
                            for (int i = 0; i < chars.length; i++) {
                                ...
                                if (!LibStringUtil.isChinese(ss) && !LibStringUtil.isNumeric(ss)
                                    ...
                                    LibPublicUtil.showToast(mContext, nameModeTxt);
                                    resetText = true;
                                    setText(tmp);
                                    setSelection(cursorPos);
                                    invalidate();
                            ...
                            if (LibStringUtil.containsEmoji(str)) {
                                LibPublicUtil.showToast(mContext, mContext.getString(R.string
                                        .content_no_emoji));
                                resetText = true;
                                setText(tmp);
                                setSelection(cursorPos);
    ...
    private String nameModeTxt;

    /**
     * 设置不支持字符错误文本
     */
    public void setNameModeTxt(String txt) {
    ...
    String getStringForName(String source) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (LibStringUtil.isChinese(codePoint) || LibStringUtil.isNumeric(codePoint) ||
                    LibStringUtil.isLetter(codePoint)
                    ...
                    || getResources().getString(R.string.limit_char_2).equalsIgnoreCase
                    (codePoint)) {
                str += codePoint;
            } else {
                str += "";
